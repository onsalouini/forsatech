        ...
        <div key={sIdx} className="rounded-xl border border-slate-200 bg-slate-50/50 p-3">
          ...
          <div className="mt-3 rounded-lg border border-slate-200 bg-white p-2.5">
            ...
            {(section.videos || []).map((video, vIdx) => (
              <div key={vIdx} className="mb-2 grid grid-cols-12 gap-2 last:mb-0">
                <input
                  placeholder="Titre vidéo"
                  className="col-span-4 rounded-md border border-slate-200 px-2 py-1.5 text-xs outline-none focus:border-cyan-500"
                  value={video.title || ''}
                  onChange={(e) => updateVideo(sIdx, vIdx, { title: e.target.value })}
                />
                <input
                  placeholder="URL (YouTube, Vimeo, etc.)"
                  className="col-span-7 rounded-md border border-slate-200 px-2 py-1.5 text-xs outline-none focus:border-cyan-500"
                  value={video.url || ''}
